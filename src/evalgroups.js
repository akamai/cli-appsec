...
class AttackGroups {
  ...
  getAttackGroupActions() {
    return this._policyProvider.policyId().then(policyId => {
      return this._version.readResource(URIs.EVAL_GROUP_ACTIONS, [policyId]);
    });
  }

  getAttackGroupAction() {
    return this._policyProvider.policyId().then(policyId => {
      return this._version.readResource(URIs.EVAL_GROUP_ACTION, [policyId, this._options['group']]);
    });
  }
  ...
  getAttackGroupException() {
    return this._policyProvider.policyId().then(policyId => {
      return this._version.readResource(URIs.EVAL_GROUP_CONDITION_EXCEPTION, [
        policyId,
        this._options['group']
      ]);
    });
  ...
  updateAttackGroupException() {
    if (fs.existsSync(this._options['file'])) {
      ...
      return this._policyProvider.policyId().then(policyId => {
        return this._version.updateResource(
          URIs.EVAL_GROUP_CONDITION_EXCEPTION,
          [policyId, this._options['group']],
          data
        );
      });
